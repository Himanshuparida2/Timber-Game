...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (Keyboard::isKeyPressed(Keyboard::Space) && !wasSpacePressed) {
            paused = !paused; // Toggle pause
            wasSpacePressed = true;
            if (paused) {
                messageText.setString("Game Paused. Press SPACE to resume");
                messageText.setPosition(850, 540); // Center pause message
                messageText.setScale(0.5, 0.5);
            } else {
